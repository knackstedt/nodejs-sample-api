...
/**
 * This list contains far more formats than what browsers can play.
...
router.post('/file', route(async (req, res, next) => {
    ...
    const only = req.query.only as "stat" | null;
    ...
    Promise.all(files.map(file => new Promise(async (resolve, reject) => {
        ...
        if (isText(file)) {
            ...
            resolve({
                ...
                type: "text",
                content: isText(null, buf) ? (only != "stat" && buf.toString()) : undefined
            });
        }
        else {
            resolve({
